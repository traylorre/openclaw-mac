# Hardening Your Mac (Intel)

This guide walks you through securing an Intel-based Mac (MacBook,
Mac Mini, iMac, or Mac Pro) using the OpenClaw hardening toolkit.
Every command is copy-pasteable. The process takes about 15 minutes.
...
- An Intel-based Mac (2009 or later)
...
On Intel Macs, Homebrew installs to `/usr/local` and is usually
available immediately — no PATH setup needed. Verify it worked:
...
If you see `command not found`, try adding Homebrew to your PATH:
...
Then try `brew --version` again. If it still fails, close Terminal
completely (Cmd + Q), reopen it, and retry.
...
[1/8] Platform Check
  ✓  macOS 13.7.8 detected
  ℹ  Architecture: x86_64

[2/8] Homebrew
  ✓  Homebrew installed: Homebrew 5.x.x
...
================================================================
  ...
  Deployment: unknown | macOS: 13.7.8
================================================================
...
[Section: Startup Security]
  WARN  No firmware password set (Intel)             → §2.9
  ...
...
**Intel-specific note:** You may see a WARN for "No firmware password
set (Intel)." This is expected. Setting a firmware password requires
booting into Recovery Mode. See
...
applied — the error message will explain why.
...
You can also compare against your initial audit to see what improved:

```bash
sudo bash scripts/hardening-audit.sh --compare /tmp/openclaw-audit.json
```

This shows a drift report listing each check that changed status.
...
- **Set a firmware password** (Intel only) — requires booting into
  Recovery Mode. See [docs/HARDENING.md §2.9](docs/HARDENING.md) for
  instructions. This prevents unauthorized booting from external
  media.
...
On Intel Macs, Homebrew installs to `/usr/local/bin/`. Try:
...
This is expected on Intel Macs. Setting a firmware password requires
booting into Recovery Mode (restart, hold Cmd + R). It is optional
but recommended for servers in shared physical spaces.
