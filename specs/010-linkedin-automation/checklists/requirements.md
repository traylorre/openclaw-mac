...
1. **Likes approval (HIGH)** — Resolved: FR-003 now distinguishes warmup (individual approval) from steady-state (batch approval). FR-012 defines warmup mode. US2 scenarios 8-9 cover both modes.
2. **Phase conflation (HIGH)** — Resolved: Added FR-009 (defensive anti-detection), updated assumption that CDP failure may require project re-evaluation. Phasing deferred to planning.
...
4. **Discovery operator controls (MEDIUM)** — Resolved: FR-011 (operating config), FR-013 (on-demand discovery). US2 scenarios 6-7 cover quiet hours and on-demand. Operating Configuration entity added.
5. **Account warmup (MEDIUM)** — Resolved: FR-012 defines warmup mode. Assumption added for benefactor warmup sequence. US2 scenario 8 covers warmup behavior.
...
- FRs expanded from 18 to 23 to cover: warmup mode (FR-012), on-demand discovery (FR-013), image support (FR-005), defensive anti-detection (FR-009), draft persistence (FR-018), and operating configuration (FR-011).
