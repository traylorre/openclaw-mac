...
- [x] T040 [US6] Update `workflows/token-check.json`: add dual-token tracking in Workflow Static Data — store both `access_token_granted_at` and `refresh_token_granted_at` timestamps. Compute days remaining for each. Existing 60-day access token logic is preserved; add 365-day refresh token tracking alongside it. Implementation notes: (1) Define mechanism for initial `refresh_token_granted_at` population — same moment as access token grant, recorded by operator after OAuth flow. (2) Consolidate duplicate Code node logic between Schedule and Webhook paths (DRY)
- [x] T041 [US6] Update `workflows/token-check.json`: add HTTP Request node for automated refresh — when access token has ≤7 days remaining, POST to `https://www.linkedin.com/oauth/v2/accessToken` with `grant_type=refresh_token`, `refresh_token` from n8n credential store, `client_id` and `client_secret` from OAuth credential. Parse response and update `access_token_granted_at` in Static Data. Implementation notes: (1) Handle `invalid_grant` (refresh token revoked/expired) → alert operator. (2) Handle `invalid_client` (client secret rotated) → distinct alert "verify client credentials in n8n." (3) Handle HTTP 5xx → retry up to 3 times with exponential backoff across the day. (4) Handle network error → retry once then alert. (5) Add `refresh_in_progress` flag in Static Data to prevent concurrent refresh from dual triggers. (6) Skip refresh when `refresh_token_expired` flag is set (circuit breaker per T042)
- [x] T042 [US6] Update `workflows/token-check.json`: add refresh token expiry monitoring — compute days remaining on 365-day refresh token. When ≤30 days remaining, POST refresh token expiry alert to OpenClaw inbound hook per `specs/010-linkedin-automation/contracts/n8n-to-openclaw-hooks.md`. When refresh token has expired, POST critical alert with manual re-authorization instructions. Implementation notes: (1) Set `refresh_token_expired` flag in Static Data when days_remaining ≤ 0 — T041 checks this as circuit breaker. (2) Update the hook contract with new alert types: `refresh_token_expiring`, `refresh_token_expired`, `token_refresh_failed`. (3) Fix existing `alert_type` inconsistency: `token_expiring` in workflow vs `token_expiry` in contract — standardize to `token_expiring`
- [x] T043 [P] [US6] Update `specs/010-linkedin-automation/research.md` R-006: correct the LinkedIn refresh token information. Note that consumer apps with `w_member_social` now support programmatic refresh tokens (365-day TTL). Reference: Microsoft Learn LinkedIn API documentation. Also update FR-016 rate limit note per research R-004 (rate limits are unpublished, must be determined empirically)
- [x] T044 [P] [US6] Update `specs/010-linkedin-automation/spec.md` Credential Lifecycle State entity: change from "60-day expiry, manual re-auth" to "60-day access token + 365-day refresh token, automated refresh". Also note in the entity description that 365-day refresh tokens increase credential theft blast radius under ASI04 residual risk (N8N_BLOCK_ENV_ACCESS_IN_NODE=false)
- [ ] T045 [US6] End-to-end verification: set access_token_granted_at to 53 days ago in n8n Static Data → trigger token-check → verify refresh HTTP request logged → verify new access_token_granted_at stored → verify refresh_token_granted_at unchanged. Implementation notes: (1) Set Static Data via n8n API: `PATCH /api/v1/workflows/{id}` with staticData payload, or edit in n8n UI. (2) LinkedIn OAuth endpoint cannot be integration-tested without valid credentials — verify workflow logic up to the HTTP Request node. (3) Test error paths by simulating invalid_grant response (if possible via n8n test framework)
