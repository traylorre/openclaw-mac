...
- [ ] T019 [P] [US2] Add `check_sensitive_file_protections()` to `scripts/hardening-audit.sh`: iterate the sensitive file inventory, verify each file's permissions, immutability flags, and HMAC signatures match expected state. Report per-file PASS/FAIL
- [ ] T020 [P] [US2] Add `check_env_gitignore()` to `scripts/hardening-audit.sh`: verify `.env` at repo root is listed in `.gitignore` and has mode 600. Verify `~/.openclaw/.env` has mode 600
- [ ] T021 [P] [US2] Add `check_lock_state_signed()` to `scripts/hardening-audit.sh`: call `integrity_verify_state_file()` for lock-state.json (existing function in lib/integrity.sh), report PASS/FAIL
- [ ] T022 [P] [US2] Add `check_heartbeat_signed()` to `scripts/hardening-audit.sh`: call `integrity_verify_state_file()` for heartbeat (existing function in lib/integrity.sh), report PASS/FAIL
- [ ] T023 [US2] Wire all US2 checks into the `run_checks_sensitive_files()` section of `scripts/hardening-audit.sh`
...
- [ ] T025 [US3] Create `docs/ASI-MAPPING.md` with all 10 OWASP ASI risks mapped per research.md R-005. Each risk: ID, name, pipeline controls, verification method (audit check name), residual risk, residual severity, remediation milestone. Include FR-026 (LLM compromise) and FR-027 (binary integrity) as documented residual risks. Include FR-019 (N8N_BLOCK_ENV_ACCESS trade-off) under ASI04 residual risk. Include note that ASI01-ASI10 are local identifiers mapped to the official OWASP Top 10 for Agentic Applications categories
- [ ] T026 [US3] Add `check_asi_controls()` to `scripts/hardening-audit.sh`: for each ASI risk, verify at least one control is active. Map to existing checks where possible (e.g., ASI03 → CHK-OPENCLAW-CREDS, ASI07 → CHK-OPENCLAW-WEBHOOK-AUTH). Report per-ASI PASS/FAIL
...
- [ ] T028 [P] [US5] Add `check_defense_layer_prevent()` to `scripts/hardening-audit.sh`: verify credential isolation (CHK-OPENCLAW-CREDS), HMAC auth (CHK-OPENCLAW-WEBHOOK-AUTH), workspace immutability (CHK-OPENCLAW-INTEGRITY-LOCK), sandbox mode (CHK-OPENCLAW-SANDBOX-MODE). Aggregate: PASS if all pass, PARTIAL if ADV-001 Keychain gap acknowledged, FAIL if any control missing
- [ ] T029 [P] [US5] Add `check_defense_layer_contain()` to `scripts/hardening-audit.sh`: verify Docker isolation (read-only FS, non-root, dropped caps), OpenClaw sandbox (ro workspace, tool deny lists), dangerous node exclusion (NODES_EXCLUDE). Aggregate status
- [ ] T030 [P] [US5] Add `check_defense_layer_detect()` to `scripts/hardening-audit.sh`: verify pre-launch attestation (integrity-verify.sh exists and runs), continuous monitoring (monitor heartbeat fresh), behavioral baseline (baseline file exists). Aggregate status
- [ ] T031 [P] [US5] Add `check_defense_layer_respond()` to `scripts/hardening-audit.sh`: verify alert delivery (webhook callback configured in openclaw.json), audit logging (make audit --json produces output), manual remediation (make integrity-lock target exists). Aggregate status
- [ ] T032 [P] [US5] Add `check_defense_layer_recover()` to `scripts/hardening-audit.sh`: verify credential rotation procedure documented (docs/DEPENDENCY-UPDATE-PROCEDURE.md exists), manifest re-baseline (make integrity-deploy works), dependency rollback (rollback section in update procedure). Aggregate status
- [ ] T033 [US5] Wire `integrity_check_env_vars()` (already implemented in lib/integrity.sh line 641) into the audit run list in `scripts/hardening-audit.sh`. This function already checks all 7 dangerous variables per FR-023. No new implementation needed — just add `run_check integrity_check_env_vars` to the pipeline security checks section
- [ ] T034 [US5] Implement behavioral baseline in `scripts/integrity-verify.sh`: on first run, record webhook call frequency (from n8n execution history API) as baseline to `~/.openclaw/behavioral-baseline.json`. On subsequent runs, compare current frequency against baseline. WARN if deviation exceeds 200%. Note: skill invocation logging deferred until OpenClaw agent is running and producing logs
- [ ] T035 [US5] Wire all US5 checks into `run_checks_defense_layers()` section of `scripts/hardening-audit.sh`
- [ ] T036 [US5] End-to-end verification: run `make audit` → confirm all 5 layers report healthy. Stop monitor → confirm Detect layer FAIL. Restart → confirm recovery
...
**Phase 3 (US1)**: T010, T011, T012 can run in parallel (different check functions)
**Phase 4 (US2)**: T018, T019, T020, T021 can run in parallel (different check functions)
**Phase 6 (US5)**: T027, T028, T029, T030, T031 can run in parallel (different defense layer checks)
