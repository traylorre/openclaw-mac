...
- **FR-004**: The system MUST capture the most recent user prompt or active task description from the session transcript during compaction detection (i.e., in the SessionStart hook handler), storing it in a temporary file for later use. Capture happens by reading the transcript file at hook execution time — not before compaction, since no pre-compaction hook exists for this event.
